...
benchmarks. It exists so local runs, CI checks, future UI views, and daemon-side
diagnostics can compare the same fields without guessing names or units.
...
| `source_id` | string or null | none | Stable fixture/source id from a manifest when provided. |
...
Threshold mode turns a benchmark into a regression check. The first production
thresholds are:
